...
for s in twitter_accounts[:len(twitter_accounts)]:
    ...
    while page < 4:
        ...
        page += 1
        if page > 4:
            print ("NO SE ENCUENTRA AL FINAL DE LA PÁGINA 2")
            break    
...
# Añadidura de los campos "followers" y "following" a cada cuenta en MongoDB.    
...
result2= tweets.aggregate([{"$group":{"_id": '$user.screen_name', "following": {"$max" : "$user.friends_count"}}}])
print(result2)
